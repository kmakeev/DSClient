...
/*
///Дополненая структура признаком int.
struct O1RequestParameterWitchInt {
    O1RequestParameterWitchInt(const QByteArray &n, const QByteArray &v, const bool &i): name(n), value(v), isInt(i) {
    }
    bool operator <(const O1RequestParameterWitchInt &other) const {
        return (name == other.name)? (value < other.value): (name < other.name);
    }
    QByteArray name;
    QByteArray value;
...
public:
    ...
    explicit O1(QNetworkAccessManager *manager, QObject *parent);
