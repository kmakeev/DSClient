...
#include "header\system\o1.h"
#include "header\system\o2replyserver.h"
#include "header\system\o2globals.h"
#include "header\system\o2settingsstore.h"

#define trace() if (1) qDebug()
// #define trace() if (0) qDebug()

O1::O1(QNetworkAccessManager *manager, QObject *parent) :
    QObject(parent) {

   setSignatureMethod(O2_SIGNATURE_TYPE_PLAINTEXT);
   manager_ = manager;
   ...
   connect(replyServer_, SIGNAL(verificationReceived(QMap<QString,QString>)),
    ...
    store_ = new O2SettingsStore(O2_ENCRYPTION_KEY);
//    store_->setGroupKey("odisg");
}
                                 ...
                                 QNetworkAccessManager::Operation operation)
{
    ...
    {
//        signature = clientSecret().toLatin1() + "&" + requestTokenSecret_.toLatin1();
...
void O1::onTokenRequestFinished() {
   ...
   //Добавлено две строчки для сохранения параметров в store_
//    setToken(requestToken_);
    setTokenSecret(requestTokenSecret_);
    trace() << "Verefy in store Token: " + token().toLatin1() + " TockenSecret:" + tokenSecret().toLatin1();
