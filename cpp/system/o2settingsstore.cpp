...
//    settings_->setValue("my", "DGSXDSHC^%cFDHV");
//    qDebug() << settings_->value("my").toString(); //для отладки
    settings_->setValue(fullKey, crypt_.encryptToString(value));
//    qDebug() << value; //для отладки
}

void O2SettingsStore::clearSettings() {
     qDebug() << "Clear Setings :(";        //для отладки
    settings_->clear();
}
